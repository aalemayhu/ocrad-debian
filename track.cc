...
void Track::set_track( const std::vector< Rectangle > & rectangle_vector )
  {
  data.clear();
  if( rectangle_vector.empty() ) return;
...
int Track::bottom( const int col ) const
  ...
  for( unsigned i = 0; i < data.size(); ++i )
    ...
    if( col <= vr.right() || i + 1 >= data.size() ) return vr.bottom( col );
    }
  return 0;
  }
...
int Track::top( const int col ) const
  ...
  for( unsigned i = 0; i < data.size(); ++i )
    {
    const Vrhomboid & vr = data[i];
    if( col <= vr.right() || i + 1 >= data.size() ) return vr.top( col );
    }
  return 0;
  }
...
int Track::vcenter( const int col ) const
  ...
  for( unsigned i = 0; i < data.size(); ++i )
    {
    const Vrhomboid & vr = data[i];
    if( col <= vr.right() || i + 1 >= data.size() ) return vr.vcenter( col );
    }
  return 0;
  }
